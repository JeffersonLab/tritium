#ifndef ROOT_TriTrack
#define ROOT_TriTrack
...
class TClonesArray;
class TriTrackingDetector;
class THaCluster;
class TriTrackID;

class TriTrack : public TObject {
...
public:
  ...
  TriTrack( Double_t x, Double_t y, Double_t theta, Double_t phi,
	    TriTrackingDetector* creator=0, TriTrackID* id=0,
	    THaPIDinfo* pid=0 )
    : TObject(),
  ...
  void              SetID( TriTrackID* id )   { fID   = id; }
  ...
  void              SetCreator( TriTrackingDetector* d ) { fCreator = d; }
...
protected:
  ...
  TriTrackingDetector* fCreator;     //! Detector creating this track
  ...
  TriTrackID*       fID;     //! Track identifier
  ...
  ClassDef(TriTrack,4)       // A generic particle track
};

//__________________ inlines __________________________________________________
inline
void TriTrack::SetD( Double_t x, Double_t y, Double_t theta, Double_t phi )
{
...
inline
void TriTrack::SetR( Double_t x, Double_t y, Double_t theta, Double_t phi )
{
  // set the coordinates in the rotated focal-plane frame
  fRX = x;
  fRY = y;
...
//_____________________________________________________________________________
inline
void TriTrack::SetTarget( Double_t x, Double_t y, Double_t theta, Double_t phi )
{
