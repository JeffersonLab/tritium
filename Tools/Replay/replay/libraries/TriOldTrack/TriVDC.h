...
class TriVDC : public TriTrackingDetector {

public:
  TriVDC( const char* name, const char* description = "",
	  THaApparatus* a = NULL );
  ...
  virtual TriVDCUVPlane* GetUpper() { return fUpper; }
  virtual TriVDCUVPlane* GetLower() { return fLower; }
  ...
  // Bits & and bit masks for TriTrack
  enum {
...
protected:
  ...
  TriVDCUVPlane* fLower;    // Lower UV plane
  TriVDCUVPlane* fUpper;    // Upper UV plane

  TClonesArray*  fUVpairs;  // Pairs of matched UV tracks (TriVDCTrackPair obj)
  ...
  void CalcFocalPlaneCoords( TriTrack* track, const ECoordTypes mode);
  void CalcTargetCoords(TriTrack *the_track, const ECoordTypes mode);
