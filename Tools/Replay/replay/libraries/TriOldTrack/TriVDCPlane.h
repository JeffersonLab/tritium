#ifndef ROOT_TriVDCPlane
#define ROOT_TriVDCPlane
...
class TriTriggerTime;

class TriVDCPlane : public THaSubDetector {

public:

  TriVDCPlane( const char* name="", const char* description="",
  ...
  TriVDCCluster* GetCluster(Int_t i) const
  { assert( i>=0 && i<GetNClusters() );
    return (TriVDCCluster*)fClusters->UncheckedAt(i); } 

  Int_t          GetNWires()    const { return fWires->GetLast()+1; }
  TClonesArray*  GetWires()     const { return fWires; }
  TriVDCWire*    GetWire(Int_t i) const
  { assert( i>=0 && i<GetNWires() );
    return (TriVDCWire*)fWires->UncheckedAt(i);}

  Int_t          GetNHits()     const { return fHits->GetLast()+1; }
  TClonesArray*  GetHits()      const { return fHits; }
  TriVDCHit*     GetHit(Int_t i) const
  { assert( i>=0 && i<GetNHits() );
    return (TriVDCHit*)fHits->UncheckedAt(i); }
...
//   Float_t *fTable;  // time-to-distance lookup table
  ...
  TriVDCTimeToDistConv* fTTDConv;  // Time-to-distance converter for this plane's wires

  THaDetector* fVDC;      // VDC detector to which this plane belongs
  
  TriTriggerTime* fglTrg; //! time-offset global variable. Needed at the decode stage
  
  virtual void  MakePrefix();
  virtual Int_t ReadDatabase( const TDatime& date );
  virtual Int_t DefineVariables( EMode mode = kDefine );

  ClassDef(TriVDCPlane,0)             // VDCPlane class
};
