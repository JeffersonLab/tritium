...
TriScintillator::TriScintillator( const char* name, const char* description,
				  THaApparatus* apparatus ) :
  TriNonTrackingDetector(name,description,apparatus)
{
  // Constructor
  fTWalkPar = 0;

  fTrackProj = new TClonesArray( "TriTrackProj", 5 );
}

//_____________________________________________________________________________
TriScintillator::TriScintillator( ) :
...
//_____________________________________________________________________________
THaAnalysisObject::EStatus TriScintillator::Init( const TDatime& date )
{
  // Extra initialization for scintillators: set up DataDest map

  if( TriNonTrackingDetector::Init( date ) )
    return fStatus;
...
//_____________________________________________________________________________
Int_t TriScintillator::ReadDatabase( const TDatime& date )
{
...
//_____________________________________________________________________________
Int_t TriScintillator::DefineVariables( EMode mode )
{
  ...
  RVarDef vars[] = {
    ...
    { "trx",    "x-position of track in det plane",  "fTrackProj.TriTrackProj.fX" },
    { "try",    "y-position of track in det plane",  "fTrackProj.TriTrackProj.fY" },
    { "trpath", "TRCS pathlen of track to det plane","fTrackProj.TriTrackProj.fPathl" },
    { "trdx",   "track deviation in x-position (m)", "fTrackProj.TriTrackProj.fdX" },
    { "trpad",  "paddle-hit associated with track",  "fTrackProj.TriTrackProj.fChannel" },
    { 0 }
  };
...
//_____________________________________________________________________________
void TriScintillator::DeleteArrays()
{
...
//_____________________________________________________________________________
inline 
void TriScintillator::ClearEvent()
{
...
//_____________________________________________________________________________
Int_t TriScintillator::Decode( const THaEvData& evdata )
{
  ...
  for( Int_t i = 0; i < fDetMap->GetSize(); i++ ) {
	...
	Warning( Here("Decode"), "%d hits on %s channel %d/%d/%d",
      ...
      // Take the first hit in time
      Int_t nHits = evdata.GetNumHits(d->crate, d->slot, chan);
      Int_t max_data = -1, data = -1;
...
//_____________________________________________________________________________
Int_t TriScintillator::ApplyCorrections( void )
{
...
//_____________________________________________________________________________
Double_t TriScintillator::TimeWalkCorrection(const Int_t& paddle,
					     const ESide side)
{
...
//_____________________________________________________________________________
Int_t TriScintillator::CoarseProcess( TClonesArray& /* tracks */ )
{
...
//_____________________________________________________________________________
Int_t TriScintillator::FineProcess( TClonesArray& tracks )
{
  ...
  for ( int i=0; i<n_track; i++ ) {
    TriTrack* theTrack = static_cast<TriTrack*>( tracks[i] );
    ...
    if ( ! CalcTrackIntercept(theTrack, pathl, xc, yc) ) { // failed to hit
      new ( (*fTrackProj)[i] )
	TriTrackProj(xc,yc,pathl,dx,pad,this);
      continue;
    }
    ...
    for ( Int_t j=0; j<fNhit; j++ ) {
      ...
      //cout<<"old s2 1:  "<<padx0<<"  "<<dpadx<<"  "<<fHitPad[j]<<"  "<<fSize[0]<<"  "<<fNelem<<endl;
      if (TMath::Abs(dx2) < TMath::Abs(dx) ) {
	pad = fHitPad[j];
	dx = dx2;  //cout<<"old s2 2:  "<<pad<<"  "<<dx2<<endl;
      }
