...
ClassImp(TriVDCUVPlane)


//_____________________________________________________________________________
TriVDCUVPlane::TriVDCUVPlane( const char* name, const char* description,
  ...
  : THaSubDetector(name,description,parent)
{
  ...
  fU = new TriVDCPlane( "u", "U plane", this );
  fV = new TriVDCPlane( "v", "V plane", this );

  // Create the UV tracks array
  fUVTracks = new TClonesArray("TriVDCUVTrack", 10); // 10 is arbitrary
}


//_____________________________________________________________________________
TriVDCUVPlane::~TriVDCUVPlane()
{
...
//_____________________________________________________________________________
THaDetectorBase::EStatus TriVDCUVPlane::Init( const TDatime& date )
{
...
//_____________________________________________________________________________
Int_t TriVDCUVPlane::MatchUVClusters()
{
  // Match clusters in the U plane with cluster in the V plane
  // Creates TriVDCUVTrack object for each pair
  ...
  if ( nu == 1 && nv == 1) {
    uvTrack = new ( (*fUVTracks)[0] ) TriVDCUVTrack();
    ...
    TriVDCPlane *p1, *p2; // Plane 1 and plane 2
    if ( nu > nv ) { 
    ...
    TriVDCCluster *p1Clust, *p2Clust; //Cluster from p1, p2 respectively
    TriVDCCluster *minClust;     //Cluster in p2 with time closest to p1Clust
    TriVDCHit     *p1Pivot, *p2Pivot;
    ...
    for (int i = 0; i < p1->GetNClusters(); i++) {
      ...
      if( !p1Pivot ) {
	Warning( "TriVDCUVPlane", "Cluster without pivot in p1, %d!", i );
	continue;
      }
      minClust = NULL;
	...
	if (timeDif < minTimeDif) {
      ...
      uvTrack = new ( (*fUVTracks)[i] ) TriVDCUVTrack();
      uvTrack->SetUVPlane(this);
...
//_____________________________________________________________________________
Int_t TriVDCUVPlane::CalcUVTrackCoords()
{
  ...
  for (int i = 0; i < nUVTracks; i++) {
    TriVDCUVTrack* track  = GetUVTrack(i);
    if( track )
      track->CalcDetCoords();
  }
  return nUVTracks;
...
//_____________________________________________________________________________
void TriVDCUVPlane::Clear( Option_t* opt )
{ 
...
//_____________________________________________________________________________
Int_t TriVDCUVPlane::Decode( const THaEvData& evData )
{
...
//_____________________________________________________________________________
Int_t TriVDCUVPlane::CoarseTrack( )
{
