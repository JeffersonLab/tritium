...
TriVDC::TriVDC( const char* name, const char* description,
		THaApparatus* apparatus ) :
  TriTrackingDetector(name,description,apparatus), fNtracks(0)
{
  // Constructor

  // Create Upper and Lower UV planes
  fLower   = new TriVDCUVPlane( "uv1", "Lower UV Plane", this );
  fUpper   = new TriVDCUVPlane( "uv2", "Upper UV Plane", this );
  if( !fLower || !fUpper || fLower->IsZombie() || fUpper->IsZombie() ) {
    Error( Here("TriVDC()"), "Failed to create subdetectors." );
    MakeZombie();
  }

  fUVpairs = new TClonesArray( "TriVDCTrackPair", 20 );
...
//_____________________________________________________________________________
THaAnalysisObject::EStatus TriVDC::Init( const TDatime& date )
{
  ...
  EStatus status;
  if( (status = TriTrackingDetector::Init( date )) ||
...
//_____________________________________________________________________________
Int_t TriVDC::ReadDatabase( const TDatime& date )
{
  // Read VDC database
...
//_____________________________________________________________________________
Int_t TriVDC::ConstructTracks( TClonesArray* tracks, Int_t mode )
{
  ...
  TriVDCUVTrack *track, *partner;
  TriVDCTrackPair *thePair;
  ...
  for( int i = 0; i < nLowerTracks; i++ ) {
	...
	continue;
      ...
      thePair = new( (*fUVpairs)[nPairs++] ) TriVDCTrackPair( track, partner );
  ...
  for( int i = 0; i < nPairs; i++ ) {
    if( !(thePair = static_cast<TriVDCTrackPair*>( fUVpairs->At(i) )) )
      continue;
    ...
    // If the 'tracks' array was given, add TriTracks to it 
    // (or modify existing ones).
    if (tracks) {

      // Decide whether this is a new track or an old track 
      // that is being updated
      TriVDCTrackID* thisID = new TriVDCTrackID(track,partner);
      TriTrack* theTrack = NULL;
      bool found = false;
      int t;
      for( t = 0; t < n_exist; t++ ) {
	theTrack = static_cast<TriTrack*>( tracks->At(t) );
  ...
  if( tracks && n_exist > n_mod ) {
    ...
    for( int i = 0; i < tracks->GetLast()+1; i++ ) {
      TriTrack* theTrack = static_cast<TriTrack*>( tracks->At(i) );
...
//_____________________________________________________________________________
void TriVDC::Clear( Option_t* opt )
{ 
...
//_____________________________________________________________________________
Int_t TriVDC::Decode( const THaEvData& evdata )
{
...
//_____________________________________________________________________________
Int_t TriVDC::CoarseTrack( TClonesArray& tracks )
{
...
//_____________________________________________________________________________
Int_t TriVDC::FineTrack( TClonesArray& tracks )
{
...
//_____________________________________________________________________________
Int_t TriVDC::FindVertices( TClonesArray& tracks )
{
  ...
  for( Int_t t = 0; t < n_exist; t++ ) {
    TriTrack* theTrack = static_cast<TriTrack*>( tracks.At(t) );
...
//_____________________________________________________________________________
void TriVDC::CalcFocalPlaneCoords( TriTrack* track, const ECoordTypes mode )
{
...
//_____________________________________________________________________________
void TriVDC::CalcTargetCoords(TriTrack *track, const ECoordTypes mode)
{
  ...
  TriSpectrometer *app = static_cast<TriSpectrometer*>(GetApparatus());
  // calculate momentum
...
//_____________________________________________________________________________
void TriVDC::CalcMatrix( const Double_t x, vector<THaMatrixElement>& matrix )
{
...
//_____________________________________________________________________________
Double_t TriVDC::CalcTargetVar(const vector<THaMatrixElement>& matrix,
...
//_____________________________________________________________________________
Double_t TriVDC::CalcTarget2FPLen(const vector<THaMatrixElement>& matrix,
				  const Double_t powers[][5])
{
...
//_____________________________________________________________________________
void TriVDC::CorrectTimeOfFlight(TClonesArray& tracks)
{
  const static Double_t v = 3.0e-8;   // for now, assume that everything travels at c

  // get scintillator planes
  TriScintillator* s1 = static_cast<TriScintillator*>
    ( GetApparatus()->GetDetector("s1") );
  TriScintillator* s2 = static_cast<TriScintillator*>
  ...
  for( Int_t t = 0; t < n_exist; t++ ) {
    TriTrack* track = static_cast<TriTrack*>( tracks.At(t) );
    ...
    for( Int_t i = 0; i < n_clust; i++ ) {
      TriVDCUVTrack* the_uvtrack = 
	static_cast<TriVDCUVTrack*>( track->GetCluster(i) );
      if( !the_uvtrack )
	continue;
...
//_____________________________________________________________________________
void TriVDC::FindBadTracks(TClonesArray& tracks)
{
...
//_____________________________________________________________________________
void TriVDC::Print(const Option_t* opt) const {
  TriTrackingDetector::Print(opt);
...
//_____________________________________________________________________________
bool TriVDC::THaMatrixElement::match(const THaMatrixElement& rhs) const
{
