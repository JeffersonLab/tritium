...
void GetRate(const string spec,  const string Com, double E0, double P0, double T0, 
		double* xb_avg, double* W2_avg, double* xs_he3,double* xs_he3_avg,double* rate_he3, double* xs_h3,double* xs_h3_avg,double* rate_h3, double* xs_h2,double* xs_h2_avg,double* rate_h2, double *rate_window){
	bool IsDebug = false;
	//bool IsDebug = true;
	/*
	   double E0 = 11.0; //GeV
    ...
    TString filename = Form("./results3/%s_rate_marathon_E%d_P%d_T%d_few.root", spec.c_str(), (int)(E0+0.5), (int)(P0+0.5), (int)(T0+0.5));
    if(IsDebug)
        filename = Form("./results3/%s_rate_marathon_E%d_P%d_T%d_debug_few.root", spec.c_str(), (int)(E0+0.5), (int)(P0+0.5), (int)(T0+0.5));
    if(spec=="BB" &&Com=="narrow")	
        filename = Form("./results3/%s_rate_marathon_E%d_P%d_T%d_narrow_few.root",spec.c_str(), (int)(E0+0.5), (int)(P0+0.5), (int)(T0+0.5));
...
int main(){
	...
	//const int N_X = 19;
	const int N_X =10;
	const double X_Bin[N_X] = { 0.59, 0.63, 0.67, 0.71, 0.75, 0.79, 0.83, 0.85, 0.87, 0.89};
...
/*    const int N_T =12;*/
    //const int Theta_Bin[N_T]={43,45,47,49,51,53,57,59,61,63,65,67};

    const int N_T =2;
    const int Theta_Bin[N_T]={69, 41};

	const double E0 = 11.0;
	double P0, T0, xbj, W2, Q2, nu;
	double xs_he3, xs_h3, rate_he3, rate_h3, xs_he3_avg, xs_h3_avg;
	double xs_h2, rate_h2, xs_h2_avg, rate_w, xbj_avg,W2_avg;
	...
	for(int j=0;j<N_T;j++){
		T0 = Theta_Bin[j];
		ofstream new_table(Form("hx_%s_table_T%d.dat",spec.c_str(), (int)T0));
		ofstream new_table1(Form("hx_%s_table_T%d_large.dat",spec.c_str(), (int)T0));
		...
		for(int i=0;i<N_X;i++){
            ...
            GetRate(spec.c_str(),"", E0, P0, T0, &xbj_avg, &W2_avg, &xs_he3, &xs_he3_avg, &rate_he3, &xs_h3,&xs_h3_avg, &rate_h3, &xs_h2,&xs_h2_avg, &rate_h2, &rate_w);
            if(spec.find("BB",0)!=string::npos)
                GetRate(spec.c_str(),"narrow", E0, P0, T0, &xbj_avg,&W2_avg, &xs_he3, &xs_he3_avg, &rate_he3_bb, &xs_h3,&xs_h3_avg, &rate_h3_bb, &xs_h2,&xs_h2_avg, &rate_h2_bb, &rate_w_bb);
            ...
            if(spec.find("BB",0)!=string::npos){
                new_table<<Form("%4s & %4.2f(%4.2f) & %4.2f(%4.2f) & %4.2f & %4.2f  & %4.2f & %6.4f &  %6.4f & %6.4f & %6.3f(%4.2f) & %6.3f(%4.2f) & %6.3f(%4.2f) & %5.2f \\\\\\hline ",
                        spec.c_str(),xbj,xbj_avg, W2,W2_avg, Q2, P0, T0, xs_h2_avg, xs_h3_avg, xs_he3_avg, rate_h2_bb,hour_h2_bb, rate_h3_bb,hour_h3_bb,rate_he3_bb,hour_he3_bb, total_hour_bb)<<endl;
                new_table1<<Form(" %4s & %4.2f(%4.2f) & %4.2f(%4.2f) & %4.2f & %4.2f  & %4.2f & %6.4f &  %6.4f & %6.4f & %6.3f(%4.2f) & %6.3f(%4.2f) & %6.3f(%4.2f) & %5.2f \\\\\\hline ",
                        spec.c_str(),xbj,xbj_avg, W2,W2_avg, Q2, P0, T0, xs_h2_avg, xs_h3_avg, xs_he3_avg, rate_h2,hour_h2, rate_h3,hour_h3,rate_he3,hour_he3, total_hour)<<endl;
            }else
                new_table<<Form(" %4s & %4.2f(%4.2f) & %4.2f(%4.2f) & %4.2f & %4.2f  & %4.2f & %6.4f &  %6.4f & %6.4f & %6.3f(%4.2f) & %6.3f(%4.2f) & %6.3f(%4.2f) & %5.2f \\\\\\hline ",
                        spec.c_str(),xbj,xbj_avg, W2,W2_avg, Q2, P0, T0, xs_h2_avg, xs_h3_avg, xs_he3_avg, rate_h2,hour_h2, rate_h3,hour_h3,rate_he3,hour_he3, total_hour)<<endl;
        }
