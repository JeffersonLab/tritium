...
inline Double_t gGet_Rad_Corr_Bump(const TString& aTarget, const Double_t aRctPt){
    //Add this feature in case the VZ-dependence Radiative Correction is big
    //It is not in used since the average radiative correction seems to work within 1% level
...
inline TString gGet_XGT2_Cut_SAMC(const TString& aArm,const TString& aTarget, const Double_t& aMom)
{
    //Not in use for cross section extraction, which uses gGet_XGT2_Acc_Cut(..) in XGT2_Cuts.h
