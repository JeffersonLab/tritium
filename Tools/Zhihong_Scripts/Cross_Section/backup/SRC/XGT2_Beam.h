...
//output->Stat_Err=(output->Value * sqrt(oCharge_Stat_Sum/2.0)); //A Factor of 1/2 is from the average of (u1,d1)
// output->Sys_Err=0;
  
//After discussed with Patricia, I assign the error to the systematic error part 12/19/2013	
  output->Stat_Err=0;
...
inline XGT2_VAR* gCal_Ne_Avg(const vector<Int_t>& aRunNoChain,const TString& aArm,Double_t& oRuntime)
{
  ...
  output->Sys_Err=sqrt(pow(U1_Stat_Err,2)+pow(Offset_Stat_Err*oRuntime,2)+pow(Runtime_Stat_Err*offset_u,2)+pow(Coeff_Stat_Err*oCharge,2))/alpha_u/(Qe*1e6);
...
inline XGT2_VAR* gCal_Ne_Single(const Int_t aRunNo,const TString& aArm,Double_t& oRuntime)
{
  ...
  output->Stat_Err=0;
  output->Sys_Err=sqrt(output->Value);
