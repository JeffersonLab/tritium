...
void s1_adcL()
{
 ...
 // Fill color
  ...
  cin >> run_number;
  TString filename;
  filename = Form("/w/halla-2/e08014/yez/Rootfiles/e07006_HRS_%d.root",run_number);
...
// TFile *f1 = new TFile( Form("/adaqfs/home/adaq/SRCs/replay/Rootfiles/e07006_HRS_%d_1.root",run_number));
// TTree *T = gROOT->GetObject("T");



  cout << T->GetEntries() << endl;
  //TCut time_cut = "TMath::Abs(L.vdc.u1.rawtime-900)<300 && TMath::Abs(L.vdc.u2.rawtime-900)<300 && TMath::Abs(L.vdc.v1.rawtime-900)<300 && TMath::Abs(L.vdc.v2.rawtime-900)<300";
  TCut time_cut = "DBB.evtypebits&(1<<3)&&DBB.edtpr[0]==0"; //Only T1 trigger, no EDTM pulse
  gStyle->SetOptStat(0);
  gStyle->SetOptFit(0);
  ...
  myfile << setiosflags(ios::left) << setw(9) << "Gain" << "   ";
  ...
  Float_t HV[12]={-1461.0, -1640.0, -1701.0, -1600.0, -1730.0, -1670.0, -1636.0, -1676.0, -1558.0, -1621.0, -1611.0, -1701.0};
  ...
  for(i=0;i<6;i++){
    c1->cd(i+1);
    TH1F *tt1 = new TH1F("tt1",Form("L-HRS S1L_0%d",i+1),200,250,800);
    T->Draw(Form("L.s1.la[%d]>>tt1",i),time_cut);
    ...
    delete tt1;
    TH1F *h1 = new TH1F("h1",Form("L-HRS S1L_0%d",i+1),100,min-100,min+1500);
    T->Draw(Form("L.s1.la[%d]>>h1",i),time_cut);
    ...
    myfile << setiosflags(ios::left) << setw(9) << setiosflags(ios::fixed) << setprecision(3) << peak_ratio[i] <<  "   ";
  ...
  c1->Print(Form("L_s1_adcL_%d.png",run_number));
  c1->Print(Form("L_s1_adcL_%d.pdf",run_number));
  
  TCanvas *c2 = new TCanvas("c2","c2",1200,1200);
  c2->Divide(3,2);
  for(i=0;i<6;i++){
    c2->cd(i+1);
    TH1F *tt2 = new TH1F("tt2",Form("L-HRS S1R_0%d",i+1),200,200,800);
    T->Draw(Form("L.s1.ra[%d]>>tt2",i),time_cut);
    ...
    max = gaus->GetParameter(1) + 4*gaus->GetParameter(2);
    delete tt2;
    TH1F *h2 = new TH1F("h2",Form("L-HRS S1R_0%d",i+1),100,min-100,min+1500);
    T->Draw(Form("L.s1.ra[%d]>>h2",i),time_cut);
    ...
    val[i+6] = peak_val[i+6]-ped_val[i+6]-10;
    num[i+6] = i+6+1;
    ...
    myfile << setiosflags(ios::left) << setw(9) << setiosflags(ios::fixed) << setprecision(3) << peak_ratio[i+6] <<  "   ";
  ...
  c2->Print(Form("L_s1_adcR_%d.png",run_number));
  c2->Print(Form("L_s1_adcR_%d.pdf",run_number));
 ...
 cerr <<"==========Ped Peaks==========="<<endl;
  ...
  TH2F *hh = new TH2F("hh","",13,0,13,100,100,500);
