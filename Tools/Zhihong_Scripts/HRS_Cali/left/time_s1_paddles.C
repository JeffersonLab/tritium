...
void time_paddles(TTree *T=0) {
  ...
  TString t1="(.5*(L.s1.lt[L.s1.t_pads[0]]+L.s1.rt[L.s1.t_pads[0]]))";
  TString t2="(.5*(L.s1.lt[L.s1.t_pads[1]]+L.s1.rt[L.s1.t_pads[1]]))";
  TString cut="L.s1.nthit==2&&(L.s1.t_pads[0]+1==L.s1.t_pads[1])";
  cut += "&&((DBB.evtypebits>>3)&1)&&DBB.edtpl[0]==0&&L.prl1.e+L.prl2.e>1000&&L.cer.asum_c>500";
  
  TString hist = Form("hs(%d,-0.5,%.1f,100,-100,100)",npaddles,npaddles-.5);
  TString sel = t1 + "-" + t2 + ":L.s1.t_pads[0]>>"+hist;
  cout << "selection is \n" << sel << "\n and the cut is \n" << cut << endl;
...
void time_lr(TTree* T=0) {
  ...
  // actually vs. L.s1.try -> position along the scint. paddles!
  // and fit to get the L-R time offsets, and collect
  // the slope

  //  TString t1="(L.s1.lt[L.s1.t_pads[0]]-L.s1.rt[L.s1.t_pads[0]]):B.ts.sh.y[0]";
  TString t1="(L.s1.lt[L.s1.t_pads[0]]-L.s1.rt[L.s1.t_pads[0]]):L.s1.try[0]";
  TString cut="L.s1.nthit==1";
  ...
  for (int i=0; i<npaddles; i++) {
    TString cuts = cut+Form("&&L.s1.t_pads[0]==%d",i);
    cBs && cBs->cd(1);
    T->Draw(t1+">>"+hist,cuts);
...
void time_s1_paddles()
{
  ...
  cin >> run_number;
  TString filename;
  filename = Form("/work/halla/e08014/disk1/Rootfiles_0818/e08014_L_%d.root",run_number);
