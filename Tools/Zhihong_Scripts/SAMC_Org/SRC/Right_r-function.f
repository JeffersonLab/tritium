      subroutine right_init_r_function
c      
...
c You must call right_init_r_function once before calling right_rfunction(which you can then
...
c
      open(11,file='./New-Right/no-collimator/line1.dat',status='old')
      open(12,file='./New-Right/no-collimator/line2.dat',status='old')
      open(13,file='./New-Right/no-collimator/line3.dat',status='old')
      open(14,file='./New-Right/no-collimator/line4.dat',status='old')
      open(15,file='./New-Right/no-collimator/line5.dat',status='old')
      open(16,file='./New-Right/no-collimator/line6.dat',status='old')
      open(17,file='./New-Right/no-collimator/line7.dat',status='old')
      open(18,file='./New-Right/no-collimator/line8.dat',status='old')
      open(19,file='./New-Right/no-collimator/line9.dat',status='old')
      open(20,file='./New-Right/no-collimator/line10.dat',status='old')
      open(21,file='./New-Right/no-collimator/line11.dat',status='old')
      open(22,file='./New-Right/no-collimator/line12.dat',status='old')
      open(23,file='./New-Right/no-collimator/dy-dd.dat',status='old')
      open(30,file='./New-Right/no-collimator/min-max.dat',status='old')
      read(23,*)dy,dd   !y and delta grid spacing
      close(23)
      !write(6,*)"initializing r-function arrays"
      open(31,file='./New-Right/no-collimator/information.dat'
     &,status='old')
      read(31,'(a24)')daet
   ...
   12 npts=i
      ...
      subroutine right_getindex(y,d,index)
...
c      write(6,*)dd,dy,testa,ddummy
      ...
      real function right_rfunction(y,d,theta,phi)
c
c  sample right_rfunction call
c   y is in meters (y0)
c   d (delta) is a fraction
c   theta and phi are radians (theta_0 and phi_0)
c      
c  sample right_rfunction call:  answer=right_rfunction(y,d,theta,phi)
c      
...
c      
     ...
     +              ,phimin(narray),phimax(narray),dy,dd,npts
      ...
      integer index
      right_rfunction=100000.
      call right_getindex(y,d,index)
...
c      
      if(dline(i).lt.right_rfunction)right_rfunction=dline(i)
  10  continue
c
c make the simple box 
c     
      dline(13)=tmax(index)-theta
      if(dline(13).lt.right_rfunction)right_rfunction=dline(13)
      dline(14)=theta-tmin(index)
      if(dline(14).lt.right_rfunction)right_rfunction=dline(14)
      dline(15)=phimax(index)-phi
      if(dline(15).lt.right_rfunction)right_rfunction=dline(15)
      dline(16)=phi-phimin(index)
      if(dline(16).lt.right_rfunction)right_rfunction=dline(16)

      else
      right_rfunction=-1000.  !default no acceptance value
      endif
      return
      end
