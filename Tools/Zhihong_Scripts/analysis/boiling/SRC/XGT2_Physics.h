		 ...
		 *aDEE = aDEE_List;
		 ...
		 if(aArm=="L"){
			 ...
			 *aMom = aMom_Left;
			 ...
			 if((aRunNo >= 3565 && aRunNo <= 3656) && abs(aAngle_Left - 25.00)<0.5)
				 *aAngle = aAngle_Left;
			 else if((aRunNo >= 3657 && aRunNo <= 3683) && abs(aAngle_Left - 21.00)<0.5)
				 *aAngle = aAngle_Left + (21.03 - 21.00);
			 else if((aRunNo >= 3684 && aRunNo <= 3708) && abs(aAngle_Left - 23.00)<0.5)
				 *aAngle = aAngle_Left + (23.00 - 23.00);
			 else if((aRunNo >= 3735 && aRunNo <= 3891) && abs(aAngle_Left - 25.00)<0.5)
				 *aAngle = aAngle_Left + (24.99 - 25.00);
			 //21 data are R-Only
			 else if((aRunNo >= 3892 && aRunNo <= 3916) && abs(aAngle_Right - 21.00)<0.5)
				 *aAngle = aAngle_Right + (21.03 - 21.00);
			 else if((aRunNo >= 3917 && aRunNo <= 4071) && abs(aAngle_Left - 28.00)<0.5)
				 *aAngle = aAngle_Left + (27.98 - 28.00); //Or 28.03 from Multi-C, 28.00 from C12
			 else if((aRunNo >= 4073 && aRunNo <= 4103) && abs(aAngle_Left - 21.00)<0.5)
				 *aAngle = aAngle_Left + (21.04 - 21.00);                                        
			 else if((aRunNo >= 4112 && aRunNo <= 4179) && abs(aAngle_Left - 23.00)<0.5)
				 *aAngle = aAngle_Left + (23.00 - 23.00);                                        
			 else if((aRunNo >= 4181 && aRunNo <= 4241) && abs(aAngle_Left - 25.00)<0.5)
				 *aAngle = aAngle_Left + (24.98 - 25.00);                                        
			 else if((aRunNo >= 4242 && aRunNo <= 4250) && abs(aAngle_Left - 21.00)<0.5)
				 *aAngle = aAngle_Left + (21.02 - 21.00);                                        
			 else if((aRunNo >= 4251 && aRunNo <= 4299) && abs(aAngle_Left - 28.00)<0.5)
		 ...
		 if(aArm=="R"){
			 ...
			 *aMom = aMom_Right;
			 ...
			 if((aRunNo >= 3565 && aRunNo <= 3656) && abs(aAngle_Right - 25.00)<0.5)
				 *aAngle = aAngle_Right;
			 else if((aRunNo >= 3657 && aRunNo <= 3683) && abs(aAngle_Right - 21.00)<0.5)
				 *aAngle = aAngle_Right + (21.04 - 21.00);
			 else if((aRunNo >= 3684 && aRunNo <= 3708) && abs(aAngle_Right - 23.00)<0.5)
				 *aAngle = aAngle_Right + (23.01 - 23.00);
			 else if((aRunNo >= 3735 && aRunNo <= 3891) && abs(aAngle_Right - 25.00)<0.5)
				 *aAngle = aAngle_Right + (25.00 - 25.00);
			 else if((aRunNo >= 3892 && aRunNo <= 3916) && abs(aAngle_Right - 21.00)<0.5)
				 *aAngle = aAngle_Right + (21.03 - 21.00);
			 else if((aRunNo >= 3917 && aRunNo <= 4071) && abs(aAngle_Right - 28.00)<0.5)
				 *aAngle = aAngle_Right + (27.99 - 28.00); 
			 else if((aRunNo >= 4073 && aRunNo <= 4101) && abs(aAngle_Right - 28.00)<0.5)
				 *aAngle = aAngle_Right + (27.99 - 28.00);                                        
			 else if((aRunNo >= 4112 && aRunNo <= 4179) && abs(aAngle_Right - 23.00)<0.5)
				 *aAngle = aAngle_Right + (23.04 - 23.00);                                        
			 else if((aRunNo >= 4181 && aRunNo <= 4241) && abs(aAngle_Right - 25.00)<0.5)
				 *aAngle = aAngle_Right + (25.00 - 25.00);                                        
			 else if((aRunNo >= 4242 && aRunNo <= 4250) && abs(aAngle_Right - 21.00)<0.5)
				 *aAngle = aAngle_Right + (21.03 - 21.00);                                        
			 else if((aRunNo >= 4251 && aRunNo <= 4299) && abs(aAngle_Right - 28.00)<0.5)
				 *aAngle = aAngle_Right + (27.99 - 28.00);                                        
			 else{
				 *aAngle = aAngle_Right;
				 cerr   <<Form("      *** ERROR - In XGT2_Physics.h, RunNo#%d is not in the table!!!",aRunNo)<<endl;
...
inline double gLoad_P0Table(const int aRunNo, const TString& aArm){
	...
	if(aArm=="L"){
		...
		while(!aInfileL.eof()){
			...
			if(aRunNo == aRunNo_List){
				aP0 = aP0_List;
				if(aP0<1.0 || abs(aP0_List-aP0_Set)>0.05 ) //To avoid bad calculation of field from Dipole
					aP0 = aP0_Set;
				break;
			}
		}
...
inline void gCheck_P0(const vector<Int_t>& aRunNoChain,const TString& aArm,const TString& aE_Tree_Name,double* aP0Chain)
{
	...
	//	aP0Chain[i] = aMom;
		...
		double aP0 = gLoad_P0Table(aRunNo, aArm.Data());
		aP0Chain[i] = aP0;	
...
inline void gCheck_E0(const vector<Int_t>& aRunNoChain,const TString& aArm,const TString& aT_Tree_Name, double* aE0Chain, double* aDEEChain)
{
	...
	for ( unsigned int i=0; i<aRunNoChain.size(); i++ )
	{
		...
		aE0Chain[i] = 3.356; //From MeV to GeV
