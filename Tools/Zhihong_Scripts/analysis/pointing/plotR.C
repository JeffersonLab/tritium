//void plot(){
{
	...
	ifstream infile(Form("%s_Yield_R_%s.dat",Target_Name.Data(),Kin_Name.Data()));
	ifstream infileD(Form("Dummy20cm_Yield_R_%s.dat", Kin_Name.Data()));
...
//			DEp[i] *= Cor_Down;
	...
	c1->cd(1);
	gPad->SetLogy(1);
	TH2F *h1 = new TH2F("h1","",300, -0.14, 0.14, 300, Y_Max*1e-5, Y_Max*1.5);
	h1->SetStats(kFALSE);
	...
	TLegend *l1 = new TLegend(0.35,0.2,0.7,0.35,Form("Yield at %s on HRS-R:",Kin_Name.Data()));
	l1->SetTextFont(32);
	l1->SetTextSize(0.03);
	l1->SetShadowColor(0);
...
//  c2->cd();
	...
	TH2F *h2 = new TH2F("h2","",300, -0.14, 0.14, 300, 85.0, 101.0);
	h2->SetStats(kFALSE);
	...
	l2->SetTextSize(0.03);
	l2->SetShadowColor(0);
	l2->AddEntry(fx,Form("%s on HRS-R",Target_Name.Data()),"p");
	l2->Draw();	
...
*/
	...
	c1->Print(Form("R_%s_Yield_%s.png",Target_Name.Data(),Kin_Name.Data()));
	c1->Print(Form("R_%s_Yield_%s.pdf",Target_Name.Data(),Kin_Name.Data()));
