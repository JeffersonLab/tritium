...
int CrossSection(const TString& aInputFile,const TString& aLogFile,const TString& aCommentName){
	...
	//////////////////////////////
	/*Get Bins{{{*/
	const TString Bin_Variable = "Ep";
	//const TString Bin_Variable = "xbj";
...
//	AccBin[2]*=2.0; //Double the Bin-Size, 2013/11/20
	...
	// SAMC
	/////////////	
	/*Get Info From SA MC Simulation{{{*/
	...
	// ///////////////
	/*Make Plots{{{*/
	//Double_t Norm = LiveTime->Value * Ntg->Value * Ne->Value * (SAMC_dOmega * P_Avg *GeVtoMeV* SAMC_Dp) / CM2ToNB/Nf_SAMC_Gen_All->Value;
	//gMake_Plot(T_Tree,ElectronCuts.Data(),SAMC_Tree,SAMC_AccCut,Target_Name.Data(),Arm.Data(),Kin.Data(),P0_Avg,Angle_Avg,Norm);
	/*}}}*/
	...
	if ( Total_Pion_Rej->Value>0&&Total_E_Eff->Value>0&&Ne->Value>0&&Ntg->Value>0){
		/*Ep{{{*/
		bin_size =  Jump*AccBin[2];
		Acc_var_Value=AccBin[0]+bin_size/2; //--> Ep
		...
		while ( Acc_var_Value<=AccBin[1] ){
			...
			Nf_EX = gGet_Nf_EX(RunNoChain,Arm,Target_Name,Kin,Ep, bin_size/2.0,ElectronCuts.Data(),Bin_Variable.Data(),LiveTimeChain,LiveTimeStatErrChain,LiveTimeSysErrChain,
					PSChain,E0Chain, P0Chain, AngleChain,IsExtTgt, Nf_EX_Chain,Nf_EX_StatChain,Nf_EX_SysChain);

			if(Target_Name == "H2"||Target_Name == "D2"||Target_Name =="He3" || Target_Name =="He4")
			//if(0)
				Nf_AL = gGet_Nf_AL(Kin, Arm,Ep, bin_size,ElectronCuts_AL.Data(),Bin_Variable.Data(),IsExtTgt);
			else{
			   Nf_AL->Value   = 1e-33;
			   Nf_AL->Sys_Err = 1e-33;
			...
			if(bCal_XS){ //Only calculate Y_MC when calculating CS
				if(Nf_EX->Value>1e-33 && Ne->Value>1e-33)
					XS_SAMC_SUM = gGet_XS_SAMC_SUM(SAMC_Tree,Ep,bin_size/2.0,SAMC_AccCut,Arm.Data(), Target_Name.Data(),Kin.Data(),P0_Avg,Angle_Avg,Bin_Variable.Data(),&BinCor);
				else
