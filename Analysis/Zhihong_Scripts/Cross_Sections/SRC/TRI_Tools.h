...
{
	public:
		/*static TRI_Logger* Instance(){{{*/
		static TRI_Logger* Instance()
		{
			// Only allow one instance of class to be generated.
			if (!m_pInstance)
			{
				Printf("@@@@@@ Creating TRI_Logger System.");
				m_pInstance = new TRI_Logger;
			}
			return m_pInstance;
		}
		/*}}}*/
		/*static TRI_Logger* Destroy(){{{*/
		static TRI_Logger* Destroy()
		{
			// Only allow one instance of class to be generated.
			if ( m_pInstance)
				delete m_pInstance;
			Printf("@@@@@@ Deleting TRI_Logger System.");
			m_pInstance=NULL;
			return m_pInstance;
		}
		/*}}}*/
	...
	private:
		TRI_Logger(){};  // Private so that it can  not be called
		TRI_Logger(TRI_Logger const&){};             // copy constructor is private
		TRI_Logger& operator=(TRI_Logger const&){};  // assignment operator is private
		static TRI_Logger* m_pInstance;
		TString LogFile_Name;
}

;/*}}}*/
TRI_Logger* TRI_Logger::m_pInstance = NULL;
...
inline void gGet_Acc_Bin(const TString& aAccBin_File,Double_t aAccBin[3],const TString& aTarget,const TString& aBin_Variable)
{
	...
	TString infilename=Form("%s/Bin/%s/%s",TRI_DIR.Data(),aBin_Variable.Data(),aAccBin_File.Data());
	TString aPath = Form("%s/Bin/%s/",TRI_DIR.Data(),aBin_Variable.Data());
