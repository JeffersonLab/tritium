...
/******************************************/
/*Include{{{*/
#include "./SRC/TRI_Main.h"
#include "./SRC/TRI_Tools.h"
#include "./SRC/TRI_LiveTime.h"
#include "./SRC/TRI_Beam.h"
#include "./SRC/TRI_Data.h"
#include "./SRC/TRI_Cuts.h"
#include "./SRC/TRI_Target.h"
#include "./SRC/TRI_Physics.h"
//#include "./SRC/TRI_Check.h"
//#include "./SRC/TRI_SAMC.h"
/*}}}*/
...
/* RunNo      --> Int_t,   First RunNumber, usally it would be the best runtime
   ...
   Kin        --> TString, Which Kinematic Settings, Kin1.0, Kin2.0 ...
   Target     --> TString, Which Target, H2,H3,He3,C12 ...
   Tgt Thick  --> Float_t, Target Thickness, g/cm2
   Thick Err  --> Float_t, Target Thickness Sys Error
   A          --> Int_t,   Target A, 
   Z          --> Int_t,   Target Z, 
   IsExtTgt   --> Int_t,   Is Extected Target? 1(Yes): H2,He3,He4, 0(No): C12
   Trigger    --> Int_t,   Main Trigger, 1,2,6 for HRS-R, 3,4,7 for HRS-L, usally we use 1 and 3 for R and L
   Scaler     --> TString, Scaler Tree, "TSLeft" for HRS-L, "TSRight" for HRS-R
...
int CrossSection(const TString& aInputFile,const TString& aLogFile,const TString& aCommentName){
	...
	//Build a logger
	TRI_Logger* log=TRI_Logger::Instance();
	...
	const Double_t Ebeam = 4.300; //GeV
	/*Read InputFile{{{*/
	...
	gCheck_LiveTime(RunNoChain,Arm,S_Tree_Name,Main_Trigger,LiveTimeChain,LiveTimeSysErrChain,PSChain);
	...
	//gCheck_Current_Avg(RunNoChain,Arm);/*}}}*/
	...
	Double_t Target_Length_Cut = 0.10; //in meter, Symmetry Cuts, like |VZ|<Target_Length_Cut
    Double_t VZ_Cut[2] ={-0.10, 0.10};
	TString AccCut_FileName = Form("%s/Acceptance/Acc_%s_Table.dat_%s",TRI_DIR.Data(), Arm.Data(),aCommentName.Data());
	////If there is not a specific acceptance file, use the default one
    if (!(gSystem->FindFile(Form("%s/Acceptance/",TRI_DIR.Data()),AccCut_FileName))){
        cout<<endl<<"******* No acceptance file is found. Use the default one!!"<<endl;
        AccCut_FileName = Form("%s/Acceptance/Acc_%s_Table.dat",TRI_DIR.Data(), Arm.Data());
    }
	...
	//if(Target_Name == "H2"||Target_Name == "D2"||Target_Name =="He3" || Target_Name =="He4"){
		//ElectronCuts_AL = gGet_TRI_Cut(RunNo,Arm,"Dummy",Kin.Data(),Angle_Avg,P,Main_Trigger,AccCut_FileName,IsExtTgt, &Target_Length_Cut); 
	...
	/////////////////////
	/*Define variables{{{*/
	TRI_VAR* Total_E_Eff=new TRI_VAR();
	TRI_VAR* Total_Pion_Rej=new TRI_VAR();
	...
	Double_t BinCor = 0.0;
	TRI_VAR* Cross_Section=new TRI_VAR();
	TRI_VAR* XS_Avg=new TRI_VAR();
	TRI_VAR* Nf_EX=new TRI_VAR();  //Total Number of real events in each bin after all cuts
	TRI_VAR* Nf_AL=new TRI_VAR();  //Total Number of real events for Dummy in each bin after all cuts
	TRI_VAR* Yield_EX=new TRI_VAR();    //Experimental Yield in this Bin
	TRI_VAR* Yield_AL=new TRI_VAR();    //Experimental Dummy Yield in this Bin
	TRI_VAR* Yield_MC=new TRI_VAR();  //SAMC Yield in this Bin
	TRI_VAR* Yield_EX_Avg=new TRI_VAR();    //Experimental Yield in this Bin
	TRI_VAR* Yield_MC_Avg=new TRI_VAR();  //SAMC Yield in this Bin
	TRI_VAR* XS_SAMC_SUM=new TRI_VAR(); //Sum of cross section values for all SAMC events in one bin

	//Double_t rms;//tmp use

	TString OutFile_Dir = Form("%s/results/%s/%s",
			TRI_DIR.Data(),Target_Name.Data(),aCommentName.Data()); 
	...
	TString outfile_name=Form("%s/results/%s/%s/%s_%s_%s_Yield.dat_%s",
			TRI_DIR.Data(),Target_Name.Data(),aCommentName.Data(),Arm.Data(),Kin.Data(),Target_Name.Data(),aCommentName.Data());
	...
	if(IsExtTgt){
		...
		Ntg= gCal_Ntg(RunNoChain,Arm.Data(),Target_A,Target_Z,Target_Thickness,Target_Thickness_Stat_Err,NtgChain);
	}
	else{
	...
	if ( Total_Pion_Rej->Value>0&&Total_E_Eff->Value>0&&Ne->Value>0&&Ntg->Value>0){
		...
		while ( Acc_var_Value<=AccBin[1] ){
			...
			Nf_EX = gGet_Nf_EX(RunNoChain,Arm,Kin,Target_Name, Main_Trigger, xbj, bin_size/2.,ElectronCuts.Data(), VZ_Cut,Bin_Variable.Data(),LiveTimeChain,LiveTimeStatErrChain,LiveTimeSysErrChain, PSChain,E0Chain, P0Chain, AngleChain,IsExtTgt, Nf_EX_Chain,Nf_EX_StatChain,Nf_EX_SysChain);
			...
			if(Nf_EX->Value>5. && Ne->Value>5. && Total_Pion_Rej->Value > 1e-33 && Total_E_Eff->Value > 1e-33){
				Yield_EX->Value = Nf_EX->Value*Total_Pion_Rej->Value/(Ne->Value*Total_E_Eff->Value) * CM2ToNB/Ntg->Value; //Eq(4.9) in page118 of Nadia's Thesis
				...
				for(int i=0;i<Chain_Size;i++){
					if(Nf_EX_Chain[i]>1){
						Y_EX_Chain[i] = Nf_EX_Chain[i]*Total_Pion_Rej->Value/(NeChain[i]*Total_E_Eff->Value) * CM2ToNB/Ntg->Value;
			...
			outlog <<endl
				<<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl;
			outlog << Form("      --> Xbj=%5.3f, Yield =%7.6e / %7.6e / %7.6e",xbj, Yield_EX->Value, Yield_EX->Sys_Err, Yield_EX->Stat_Err)<<endl;
			outlog <<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl<<endl;   
			cout <<endl
				<<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl;
			cout << Form("      --> Xbj=%5.3f, Yield =%7.6e / %7.6e / %7.6e",xbj, Yield_EX->Value, Yield_EX->Sys_Err, Yield_EX->Stat_Err)<<endl;
			cout <<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl<<endl;   
			NoCal++;
			NumOfAccInput++;
