...
class SAMCEvent
{
	...
	private:
		...
		int ReconstructTg(const double& ax,const double& ay,const double& ath,const double& aph,const double& axtg)
		{
			...
			/* double Angle_rec = Angle+atan(th_tg_rec); */
			Angle_rec = Angle;
	...
	private:
		...
		double _sigma_q(const double& aEs,const double& aEp)
		{
			...
			double lsigma_q=sigma_qfs(Target.Z,Target.A,aEs,aEs-aEp,Angle_Deg,Fermi_Moment,NIE,IsQFS_Q2dep);
	...
	public:
		...
		double Angle_rec ; //Scattering Angle after reconstruction in Rad 
