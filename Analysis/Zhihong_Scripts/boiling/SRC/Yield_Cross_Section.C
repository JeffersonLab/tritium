...
int CrossSection(const TString& aInputFile,const TString& aLogFile){
	...
	XGT2_VAR* Yield_SAMC=new XGT2_VAR();  //SAMC Yield in this Bin
	XGT2_VAR* XS_SAMC_SUM=new XGT2_VAR(); //Sum of cross section values for all SAMC events in one bin

	//Double_t rms;//tmp use
	TString outfile_name=Form("%s/results/%s/%s_%s_%d_Yield.dat",XGT2_DIR.Data(),Target_Name.Data(),Arm.Data(),Kin.Data(),RunNo);
	...
	TString outstring=Form("%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s",
				...
				while ( Acc_var_Value[1]<=AccBin[1][1] ){
					...
					while ( Acc_var_Value[2]<=AccBin[2][1] ){
						...
						while ( Acc_var_Value[3]<=AccBin[3][1] ){
							...
							// Nf_SAMC->Value = gGet_Nf_SAMC(SAMC_Tree,Acc_var_Value,SAMC_BinCut, SAMC_AccCut,Target_Name.Data(),Kin.Data(),Ep/1000.0);
							...
							/*Yield{{{*/
							Yield_EX->Value = Nf_EX->Value*Total_Pion_Rej->Value/(Ne->Value*Total_E_Eff->Value); //Eq(4.9) in page118 of Nadia's Thesis
							Yield_EX->Stat_Err = Yield_EX->Value * sqrt( pow(Nf_EX->Stat_Err/Nf_EX->Value,2) + pow(Ne->Stat_Err/Ne->Value,2) ); 
							Yield_EX->Sys_Err  = 0.0; //FIX_HERE, set zero so far

							XS_SAMC_SUM->Value = gGet_XS_SAMC_SUM(SAMC_Tree,Acc_var_Value,SAMC_BinCut, SAMC_AccCut,Target_Name.Data(),Kin.Data());
							XS_SAMC_SUM->Stat_Err = 0.0;//FIX_HERE, don't know how to estimate the statistics errors
							XS_SAMC_SUM->Sys_Err = 0.0; //Should be Zero
							Yield_SAMC->Value =  Ntg->Value*XS_SAMC_SUM->Value*(SAMC_dOmega*(P_Center*SAMC_dDp)/Nf_SAMC_Gen_All->Value); //Eq(4.18) in page 130 of Nadia's Thesis,set Det Effi to one and quote its sys err later
							Yield_SAMC->Stat_Err = Yield_SAMC->Value 
							...
							Yield_SAMC->Sys_Err = 0.0; //FIX_HERE, set zero so far
							outlog << Form( "       Yield_EX=%e, Yield_SAMC=%e", Yield_EX->Value, Yield_SAMC->Value)<<endl;
							...
							///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
							///////////////////////////////////////////////////////////////////////////////////////////////////////////////////																		  
							if ( Yield_SAMC->Value>0 && Yield_EX->Value>0){
								...
								Cross_Section->Value = Yield_EX->Value/Yield_SAMC->Value * gGet_XS_Born_XEMC(Target_Name.Data(), Kin.Data(), Ebeam/1000.0, Ep/1000.0, Angle);
								///////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
								///////////////////////////////////////////////////////////////////////////////////////////////////////////////////

								Cross_Section->Stat_Err=Cross_Section->Value 
                    									* sqrt( pow(Yield_EX->Stat_Err   / Yield_EX->Value,2)
					            						+       pow(Yield_SAMC->Stat_Err / Yield_SAMC->Value,2)	);
								Cross_Section->Sys_Err=Cross_Section->Value 
								                      * sqrt( pow(Yield_EX->Sys_Err   / Yield_EX->Value,2)
											          +       pow(Yield_SAMC->Sys_Err / Yield_SAMC->Value,2)	);
								outstring=Form("%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e",
										...
										16,Yield_EX->Sys_Err,
										16,Yield_SAMC->Value,
										16,Yield_SAMC->Stat_Err,
										16,Yield_SAMC->Sys_Err
										);
								outfile<<outstring<<endl;
								outlog <<endl
									<<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl;
								outlog << Form("      --> Xbj=%5.3f, Cross_Section=%e",Acc_var_Value[3], Cross_Section->Value)<<endl;
								outlog <<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl<<endl;   
								cerr <<endl
									<<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl;
								cerr << Form("      --> Xbj=%5.3f, Cross_Section=%e",Acc_var_Value[3], Cross_Section->Value)<<endl;
								cerr <<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl<<endl;   
