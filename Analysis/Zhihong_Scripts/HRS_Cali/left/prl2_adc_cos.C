...
//   T->Add("../Rootfiles/e07006_HRS_2842_1.root");
  ...
  Double_t ped[34]={  483,   463,   453,   586,   688,   630,   653,   491,   489,   
		      414,   460,   671,   610,   633,   628,   642,   654,   464,
		      503,   519,   447,   473,   579,   651,   616,   631,   541,
		      518,   489,   536,   679,   653,   473,   431};
  TLatex *tex;
  ofstream myfile;
  ...
  for(i=0;i<16;i++){
    ...
    TH1F *tt1 = new TH1F("tt1",Form("PRL2_%d",i+1),200,30.,500);
    T->Draw(Form("L.prl2.a_p[%d]>>tt1",i));//,Form("L.prl2.a_p[%d]>100.0",i));
    max_bin = tt1->GetMaximumBin();
    min = tt1->GetBinCenter(max_bin) - 50;
...
//     T->Draw(Form("L.prl2.a_p[%d]>>h1",i));
//     //    T->Draw(Form("L.prl2.a_p[%d]>>h1",i),Form("L.prl2.a_p[%d]>100.0&&L.cer.asum_p<400.&&L.prl2.a<600.",i));
...
//     peak_val = landau->GetParameter(1);
  ...
  c1->Print(Form("L_prl2_adj1_%d.png",run_number));
  ...
  for(i=0;i<16;i++){
    c2->cd(i+1);
    TH1F *tt2 = new TH1F("tt2",Form("PRL2_%d",i+17),200,30,500);
//     T->Draw(Form("L.prl2.a_p[%d]>>tt2",i+16),Form("L.prl2.a_p[%d]>100.0&&L.cer.asum_p<400.&&L.prl2.a<600.",i+16));
    T->Draw(Form("L.prl2.a_p[%d]>>tt2",i+16));

    max_bin = tt2->GetMaximumBin();
    min = tt2->GetBinCenter(max_bin) - 50;
...
//     T->Draw(Form("L.prl2.a_p[%d]>>h2",i+16),Form("L.prl2.a_p[%d]>100.0",i+16));
...
//    peak_val = landau->GetParameter(1);
  ...
  c2->Print(Form("L_prl2_adj2_%d.eps",run_number));
  c2->Print(Form("L_prl2_adj2_%d.png",run_number));
  ...
  for(i=0;i<2;i++){
    c3->cd(i+1);
    TH1F *tt3 = new TH1F("tt3",Form("PRL2_%d",i+33),200,30,500);
    T->Draw(Form("L.prl2.a_p[%d]>>tt3",i+32));//,Form("L.prl2.a_p[%d]>100.0",i+32));
    max_bin = tt3->GetMaximumBin();
    min = tt3->GetBinCenter(max_bin) - 50;
    max = tt3->GetBinCenter(max_bin) + 50;
    tt3->Fit("gaus","","",min,max);
   //  min = gaus->GetParameter(1) - 4*gaus->GetParameter(2);
//     max = gaus->GetParameter(1) + 4*gaus->GetParameter(2);
//     delete tt3;
    
//     TH1F *h3 = new TH1F("h3",Form("PRL2_%d",i+33),100,min-40,min+600);
//     //    T->Draw(Form("L.prl2.a_[%d]>>h3",i+32),Form("L.prl2.a_p[%d]>100.0&&L.cer.asum_p<600.",i+32));
//     T->Draw(Form("L.prl2.a_p[%d]>>tt2",i+32));
