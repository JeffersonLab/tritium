...
void time_paddles(TTree *T=0) {
  ...
  TString t1="(.5*(R.s2.lt[R.s2.t_pads[0]]+R.s2.rt[R.s2.t_pads[0]]))";
  TString t2="(.5*(R.s2.lt[R.s2.t_pads[1]]+R.s2.rt[R.s2.t_pads[1]]))";
  TString cut="R.s2.nthit==2&&(R.s2.t_pads[0]+1==R.s2.t_pads[1])";
  ...
  TString sel = t1 + "-" + t2 + ":R.s2.t_pads[0]>>"+hist;
  cout << "selection is \n" << sel << "\n and the cut is \n" << cut << endl;
...
void time_lr(TTree* T=0) {
  ...
  TString t1="(R.s2.lt[R.s2.t_pads[0]]-R.s2.rt[R.s2.t_pads[0]]):R.s2.try[0]";
  TString cut="R.s2.nthit==1";
  cut += "&&R.tr.n==1&&R.vdc.u1.nclust==1&&R.vdc.u2.nclust==1&&R.vdc.v1.nclust==1&&R.vdc.v2.nclust==1";
  cut += "&&((DBB.evtypebits>>1)&1)&&DBB.edtpr[0]==0&&(R.ps.e+R.sh.e)>1200&&R.cer.asum_c>500";
  ...
  for (int i=0; i<npaddles; i++) {
    TString cuts = cut+Form("&&R.s2.t_pads[0]==%d",i);
...
void time_s2m_paddles()
{
  ...
  cin >> run_number;
  TString filename;
  //  filename = Form("../replay/Rootfiles/e08014_R_%d.root",run_number);
  filename = Form("/GoFlex/Rootfiles/e08014_R_%d.root",run_number);
  TChain *T = new TChain("T");
  T->Add(filename);
  filename = Form("/GoFlex/Rootfiles/e08014_R_%d.root_1",run_number);
  T->Add(filename);
  ...
  T->Add(filename);
  filename = Form("/GoFlex/Rootfiles/e08014_R_%d.root",run_number+2);
  T->Add(filename);
  filename = Form("/GoFlex/Rootfiles/e08014_R_%d_1.root",run_number+2);
  T->Add(filename);
  filename = Form("/GoFlex/Rootfiles/e08014_R_%d.root",run_number+3);
  T->Add(filename);
  filename = Form("/GoFlex/Rootfiles/e08014_R_%d_1.root",run_number+3);
  T->Add(filename);
